...
class WSIReader:
    ...
    def open(input_img):
        ...
        if isinstance(input_img, (str, pathlib.Path)):
            ...
            if suffixes[-1] in (".npy",):
                ...
                return VirtualWSIReader(input_img)
            if suffixes[-2:] in ([".ome", ".tiff"],):
                return TIFFWSIReader(input_img)
            if suffixes[-1] in (".jpg", ".png", ".tif"):
                return VirtualWSIReader(input_img)
            if suffixes[-1] in (".svs", ".ndpi", ".mrxs"):
                return OpenSlideWSIReader(input_img)
            if suffixes[-1] == (".jp2"):
                return OmnyxJP2WSIReader(input_img)
            raise FileNotSupported("Filetype not supported.")
        if isinstance(input_img, np.ndarray):
            return VirtualWSIReader(input_img)
        if isinstance(input_img, WSIReader):
            # Input is already a tiatoolbox WSIReader
            return input_img
    ...
    def _bounds_at_resolution_to_baseline(self, bounds, resolution, units):
        ...
        return np.concatenate([tl_at_baseline, br_at_baseline])
    ...
    def _find_tile_params(self, tile_objective_value: int):
        ...
        if not rescale.is_integer():
            raise ValueError(
                "Tile objective value must be an integer multiple "
                "of the objective power."
            )
        try:
    ...
    def tissue_mask(
        ...
        if method not in ("otsu", "morphological"):
            raise ValueError("method must be either 'otsu' or 'morphological'")
        if method == "otsu":
    ...
    def save_tiles(
        ...
        x_tiles_count = int(math.ceil((slide_w - tile_w) / tile_w + 1))
        y_tiles_count = int(math.ceil((slide_h - tile_h) / tile_h + 1))
        tiles_shape = (x_tiles_count, y_tiles_count)
        for i, (h, w) in enumerate(np.ndindex(tiles_shape)):
            start_h = h * tile_h
...
class OmnyxJP2WSIReader(WSIReader):
    ...
    def __init__(self, input_img):
        ...
        # Enable OpenJPEG multi-threading
...
class TIFFWSIReader(WSIReader):
    def __init__(self, input_img, series="auto", cache_size=2 ** 28):
        ...
        # Wrap in view classes to re-order indexes for reading.
        # Could be done with zarr `array.view` but this currently gives
        # a ValueError when used with tifffile.
        self.level_arrays = {
