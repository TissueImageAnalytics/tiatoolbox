...
####
def _postproc(pred_map, nr_types=6, return_centroids=True):
    ...
    if return_centroids or nr_types is not None:
        ...
        for inst_id in inst_id_list:
            ...
            inst_bbox = np.array([[cmin, rmin], [cmax, rmax]])
            inst_map = inst_map[
            ...
            inst_contour[:, 0] += inst_bbox[0][0]  # X
            inst_contour[:, 1] += inst_bbox[0][1]  # Y
            inst_centroid[0] += inst_bbox[0][0]  # X
            inst_centroid[1] += inst_bbox[0][1]  # Y
...
class HoVerNet(ModelBase):
    ...
    def set_postproc_func(self, func=None):
        ...
        """
        self.postproc_func = func if func is not None else _postproc
