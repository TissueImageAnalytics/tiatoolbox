...
class PatchPredictor:
    ...
    def merge_predictions(
        ...
        Args:
            input_img (:obj:`str` or :obj:`pathlib.Path` or :class:`numpy.ndarray`):
                Image to be processed. This can be a WSI, tile or patch.
            output (dict):
    ...
    def _predict_patch(
        ...
        if labels and len(labels) != len(input_imgs):
            raise ValueError(
                f"len(labels) != len(imgs) : " f"{len(labels)} != {len(input_imgs)}"
            )

        # don't return coordinates if patches are already extracted
        return_coordinates = False
        dataset = PatchDataset(input_imgs, labels)
    ...
    def _predict_tile_wsi(
        ...
        for idx, input_img in enumerate(input_imgs):
            ...
            if save_output:
                img_id, save_info = self._save_output(
                    output_model,
                    idx,
                    merged_prediction,
                    input_img,
    ...
    def _save_output(
        ...
        if isinstance(input_img, WSIReader):
            img_id = str(input_img.input_path)

        elif isinstance(input_img, (str, pathlib.Path)):
            img_id = str(input_img)
    ...
    def predict(
        ...
        Args:
            ...
            stride_shape (tuple):
                ...
                `stride_shape=patch_input_shape`.
            resolution (float):
                Resolution used for reading the images. Please see
                :obj:`WSIReader` for details.
            units (str):
                Units of resolution used for reading the image. Choose
                from either `level`, `power` or `mpp`. Please see
