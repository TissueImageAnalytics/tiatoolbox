...
def get_tissue_regions(target_I, target_M, source_I, source_M, isDisplay):
	...
	target_maxr = target_maxr + math.ceil((target_maxr - target_minr) / 2) * 2 - (target_maxr - target_minr)
	target_maxc = target_maxc + math.ceil((target_maxc - target_minc) / 2) * 2 - (target_maxc - target_minc)
	...
	source_maxr = source_maxr + math.ceil((source_maxr - source_minr) / 2) * 2 - (source_maxr - source_minr)
	source_maxc = source_maxc + math.ceil((source_maxc - source_minc) / 2) * 2 - (source_maxc - source_minc)

	minc, maxc, minr, maxr = np.min([target_minc, source_minc]), np.min([target_maxc, source_maxc]), np.min([target_minr, source_minr]), np.min([target_maxr, source_maxr])
	...
	return target_tissue_I, target_tissue_M, source_tissue_I, source_tissue_M, (minr, minc, maxr, maxc)
...
class RegistrationParameters():
	...
	feature_extractor = IntermediateLayerGetter(
		model.features, return_layers={"16": "block3_pool", "23": "block4_pool", "30": "block5_pool"}
	)
...
class Registration():
	...
	def feature_mapping(cls, features, x_scale, y_scale):
		def pairwise_distance(X, Y):  # for this function shape of M and N should be same
	...
	def perform_prealignment(cls, target_I, source_I, target_M, source_M, param):
		...
		for i in range(len(list_angles)):
			...
			combinedT_COM = np.matmul(np.matmul(np.matmul(originT_COM, R), originT_COM_), comT)
			warped_source_M = cv2.warpAffine(source_M, combinedT_COM[0:-1][:], target_I.shape[:2][::-1])
	...
	@classmethod
	def perform_refinement(cls, target_I, source_I, target_M, source_M, param, initial_transform):
		...
		def show_matching_points(target, source, X, Y, dist):
			...
			img3 = cv2.drawMatches(target, keypoints1, source, keypoints2, matchingPoints, None, flags=2)
			return img3
		...
		multiscale_features, x_scale, y_scale = Registration.extract_features(target_I, source_I, param)
		target_points, source_points, dist = Registration.feature_mapping(multiscale_features, x_scale, y_scale)
		...
		filtered_target_points, filtered_source_points, filtered_dist = remove_points_using_mask(target_M,
																								 target_points,
																								 source_points,
																								 dist, 1)
		...
		return Registration.estimate_affine_transform(filtered_target_points, filtered_source_points)  # compute transformation params, if removeScale == 1, remove the scale params
	...
	def run_registration(target_wsi_path, source_wsi_path, param):
		...
		# Pre-alignment
		pre_transform = Registration.perform_prealignment(target_I, source_I, target_M, source_M, param)
		warped_source_I = cv2.warpAffine(source_I, pre_transform[0:-1][:], target_I.shape[:2][::-1])
		warped_source_M = cv2.warpAffine(source_M, pre_transform[0:-1][:], target_I.shape[:2][::-1])
		[target_tissue_I, target_tissue_M, source_tissue_I, source_tissue_M, tissue_top_left_coor] = get_tissue_regions(target_I,
																													target_M,
																													warped_source_I,
																													warped_source_M,
																													0)
		...
		tissue_transform = Registration.perform_refinement(target_tissue_I, source_tissue_I, target_tissue_M, source_tissue_M, param, pre_transform)
