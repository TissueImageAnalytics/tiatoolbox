...
Shows how to train a patch classifier using the new training package in `tiatoolbox.models.training` for the folder-based classification setup, where each class corresponds to a subfolder of patches. The notebook covers dataset construction, `DataLoaderConfig`/`create_dataloader`, trainer configuration, checkpointing, and resuming training from `last.ckpt`.
