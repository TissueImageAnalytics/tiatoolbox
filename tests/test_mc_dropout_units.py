...
# ----------------------------------------------------------------------------
# Small dummy models for testing the MC Dropout utilities without needing a full CNN or WSI.
# ----------------------------------------------------------------------------
...
class DummyCNNModel(nn.Module):
    ...
    def __init__(self, with_dropout: bool = False) -> None:
        ...
        self.classifier = nn.Sequential(nn.Dropout(0.5), linear) if with_dropout else linear
    ...
    @staticmethod
    def infer_batch(model: "DummyCNNModel", batch_data: torch.Tensor, device: str = "cpu") -> torch.Tensor:
...
def test_decompose_uncertainty_zero_variance_gives_zero_epistemic() -> None:
    ...
    mc_probs = single_pass.unsqueeze(0).repeat(10, 1, 1)  # [T=10, B=1, C=3], all identical
...
def test_bayesian_wrapper_raises_without_dropout() -> None:
    ...
    try:
        ...
        msg = "Expected ValueError when wrapping a model with no Dropout layers."
