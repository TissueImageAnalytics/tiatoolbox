...
def test_wsireader_slide_info_jp2(_response_jp2):
    """pytest for slide_info in WSIReader class as a python function"""
    file_types = ("*.jp2",)
    files_all = utils.misc.grab_files_from_dir(
        input_path=str(pathlib.Path(__file__).parent),
...
def test_save_tiles(_response_ndpi, _response_svs):
    ...
    shutil.rmtree(
        pathlib.Path(
            __file__).parent.joinpath("tiles_save_tiles"))
...
def test_save_tiles_unwrap(_response_svs):
    ...
    assert (
        pathlib.Path(
            __file__).parent.joinpath(
            "tiles_save_tiles").joinpath(
            "CMU-1-Small-Region.svs").joinpath(
            "Output.csv").exists()
    )
    assert (
        pathlib.Path(
...
def test_command_line_slide_thumbnail(_response_ndpi):
    ...
    assert pathlib.Path(__file__).parent.joinpath(
