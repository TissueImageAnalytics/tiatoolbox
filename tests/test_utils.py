...
from tiatoolbox.utils.transforms import convert_resolution, locsize2bounds
...
def test_convert_resolution():
    ...
    with pytest.raises(ValueError, match=r".*Invalid input_unit.*"):
        convert_resolution(0, input_unit="invalid")

    # Test for missing information
    with pytest.raises(ValueError, match=r".*Missing baseline_mpp.*"):
        convert_resolution(0, input_unit="mpp", baseline_mpp=None)
    with pytest.raises(ValueError, match=r".*Missing baseline_power.*"):
        convert_resolution(0, input_unit="power", baseline_power=None)
