...
def test_dict_to_store_semantic_segment():
    """Tests multipoint behaviour in dict_to_store."""
    test_pred = np.zeros(shape=(224, 224))

    patch_output = {"predictions": test_pred}

    store_  = misc.dict_to_store_semantic_segmentor(patch_output=patch_output,
    ...
    annotations_geometry_type = [str(annotation_.geometry_type) for annotation_ in
                               annotations_]

    assert "Point" in annotations_geometry_type
    assert not "Polygon" in annotations_geometry_type
