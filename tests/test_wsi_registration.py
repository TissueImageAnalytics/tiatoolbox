...
from tiatoolbox.tools.registration.wsi_registration import DFBR
...
def test_extract_features():
	...
	df = DFBR()
	with pytest.raises(ValueError, match=r".*The required shape for fixed and moving images is n x m x 3.*"):
	...
	with pytest.raises(ValueError, match=r".*The input images are expected to have 3 channels.*"):
		_ = df.extract_features(fixed_img, moving_img)
