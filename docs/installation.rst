...
Please note that TIAToolbox is tested for Python versions 3.12, 3.13, 3.14 and 3.15. For the full range of supported Python versions, please refer to the package metadata (for example, the ``python_requires`` setting in ``setup.py`` or ``pyproject.toml``)
